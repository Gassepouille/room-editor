...
export default class ToolWall extends ToolSuper {
	constructor() {
		super();
		let _geometry = new THREE.BoxBufferGeometry(2, 0.5, 0.1);
	...
	start(vectorPosition){		
		...
		// this._object3d.position.x = vectorPosition.x + 0.1/2;
	...
	update(vectorPosition){
		...
		this._object3d.position.x = this._origin.x + Math.cos(_angle);
		this._object3d.position.z = this._origin.z + Math.sin(_angle);
	}
